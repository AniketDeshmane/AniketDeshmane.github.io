...
@Component({
  ...
  viewProviders: [provideIcons({ lucideMail, lucidePhone, lucideGithub })],
  template: `
    <section class="min-h-screen flex items-center justify-center bg-background relative overflow-hidden">
      <div class="container mx-auto px-6 sm:px-8 lg:px-12 text-center relative z-10 py-20 sm:py-24 lg:py-32">
        <div class="max-w-3xl mx-auto animate-fade-in">
          ...
          <!-- Subtitle -->
          <p class="text-lg sm:text-xl md:text-2xl text-muted-foreground font-medium mb-4 sm:mb-5">
            {{ config?.subtitle || 'Software Engineer | C# | Angular | AWS' }}
          </p>

          <!-- Description -->
          <p class="text-base sm:text-lg text-muted-foreground/70 leading-relaxed mb-10 sm:mb-14 max-w-2xl mx-auto">
          ...
          <!-- CTA Buttons -->
          <div class="flex flex-col sm:flex-row flex-wrap gap-3 sm:gap-4 justify-center mb-12 sm:mb-16">
            <app-button 
              variant="default" 
              size="lg"
          ...
          <!-- Social Links -->
          <div class="flex justify-center items-center gap-5 sm:gap-6">
            <a 
              href="tel:+919619663812" 
              class="text-muted-foreground hover:text-primary transition-colors duration-300 p-2 rounded-xl hover:bg-muted/50"
              aria-label="Phone">
